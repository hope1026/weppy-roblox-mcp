...
- `manage_studio.playtest_action_execute`: run one profile Action with complete final arguments and semantic request identity. Include configured fixed input values when making the request; the Test Agent validates the actual method schema without injecting Profile defaults or comparing fixed values. Fixed inputs control the Dashboard form, not runtime authorization. Reusing the same request returns its existing result, while a new request runs the Action again.
...
The Dashboard Playtest page has two workspaces. **Test Values** manages Place-scoped profiles and separates compact read-only observations, values you can change, and Actions you run explicitly. Reorder the sections and move or pin individual entries to keep the most useful tests visible. Action forms validate declared arguments and each click creates a new execution request. Search, value-type filters, and 25, 50, or 100-row pages remain available for larger profiles. The Dashboard remembers the profile and layout for each Place. Start and stop Play or Run in Studio Editor. The session bar shows the current state and automatically attaches the selected profile to the active runtime. **Automated Test Results** provides search, result filters, pagination, and the structured or Raw Luau report viewer. Open **Before publishing** from the page header when you need the read-only publishing reference. The last selected workspace returns after reopening the page or reloading the Dashboard.

Use **Add value** and start with **Studio selection**. Choose a real Attribute, `ValueBase.Value`, supported property, or detected Humanoid and add it as an observation, control, increment Action, or kill Action without creating test code. **Test Adapter** shows detected adapters and selectable entries, including game-specific Actions with declared parameters. An adapter with no selectable entry remains visible with its method summary and **Configure values** action. Expand **Connect a value** in the same workspace when an internal value or domain operation needs a reviewed getter, setter, or Action that an Instance cannot represent. If no profile exists, the first entry and profile name are saved in the same flow. Basic includes profile creation and runtime observation. Pro adds typed value changes, explicit Action execution, rollback, Reset all, and managed Adapter authoring. Reset all restores control values; it does not undo deaths, rewards, drops, saved data, or external requests created by an Action.
