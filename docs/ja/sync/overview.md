...
Sync データは `{projectRoot}/weppy-project-sync/place_{placeId}/explorer` 配下に生成されます。
加えて、WEPPY は place ごとの sourcemap を `{projectRoot}/weppy-project-sync/place_{placeId}/sourcemap.json` に書き出し、推奨されるルート代表ファイルを `{projectRoot}/weppy-project-sync/sourcemap.json` に維持します。
`luau-lsp` などのエディタ連携ではルートパスの利用を推奨します。設定手順は [WEPPY Sync で `luau-lsp` を使う](./luau-lsp.md) を参照してください。
...
| 状態/履歴API | 非対応 | 対応 (`status_current_place`, `history`, `progress`) |
...
| アクション | 説明 | 主な引数 |
|------|------|-----------|
| `status_current_place` | 現在接続中のPlaceの同期状態を確認 | `-` |
...
- [WEPPY Sync で `luau-lsp` を使う](./luau-lsp.md)
