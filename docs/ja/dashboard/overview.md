...
| レベル | 条件 | 使用可能なページ |
|:----:|------|-------------------|
| **L0** | サーバー未接続 | 再接続待機画面のみ表示 |
| **L1** | サーバー接続、プラグイン未接続 | Connection, Tools, Settings |
| **L2** | サーバー + プラグイン両方接続 | すべてのページ |

プラグインが接続されていない状態（L1）では、Overview、Changelog、Sync、Playtestページにアクセスできません。
