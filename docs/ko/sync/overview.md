...
Sync 데이터는 `{projectRoot}/wrox-project-sync/place_{placeId}/explorer` 아래에 생성됩니다.
WROX는 여기에 더해 place별 sourcemap을 `{projectRoot}/wrox-project-sync/place_{placeId}/sourcemap.json`에 기록하고, 권장 루트 대표 파일을 `{projectRoot}/wrox-project-sync/sourcemap.json`에 유지합니다.
`luau-lsp` 같은 에디터 연동에는 루트 경로 사용을 권장합니다. 설정 방법은 [WROX Sync에서 `luau-lsp` 사용하기](./luau-lsp.md)에서 확인할 수 있습니다.

### VSCode에서 Sync 데이터 탐색

[WROX Roblox Explorer](../installation/roblox-explorer.md) 확장을 설치하면, 동기화된 인스턴스 트리를 Roblox Studio와 동일한 형태로 VSCode에서 탐색할 수 있습니다.
Explorer는 여기서 생성된 sync 파일을 읽고, 로컬 MCP 서버가 실행 중이면 추가로 실시간 sync 상태와 direction 정보를 표시할 수 있습니다.

![WROX Roblox Explorer — 동기화된 인스턴스 트리를 VSCode에서 탐색하는 모습](../../assets/screenshots/roblox-explorer/roblox-explorer-screen.png)
...
- [WROX Sync에서 `luau-lsp` 사용하기](./luau-lsp.md)
