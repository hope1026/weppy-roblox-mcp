...
> **WEPPY**는 AI 코딩 에이전트가 라이브 Roblox Studio 세션을 제어할 수 있게 하는 MCP 서버입니다 — 자연어로 스크립트, 인스턴스, 지형, 조명, 에셋, 오디오, 애니메이션을 생성하고 편집합니다.
...
## WEPPY (Weppy Roblox MCP)를 쓰는 이유
...
**WEPPY**는 AI 에이전트와 Roblox Studio를 연결하는 브릿지입니다. AI가 Studio 안에서 인스턴스, 스크립트, 속성, 지형 등을 직접 생성하고 수정하면, 변경 내용이 Studio와 대시보드에 즉시 반영되어 **무엇이 바뀌었는지 한눈에 확인**할 수 있습니다.
...
![WEPPY 플레이테스트 대시보드 — 테스트 기록 및 상세 리포트](../assets/screenshots/dashboard/dashboard_playtest.png)

### 4) WEPPY Dashboard: AI 작업을 실시간으로 모니터링
...
### 5) WEPPY Roblox Explorer: VSCode에서 Studio 계층 구조 탐색

Roblox Studio의 전체 인스턴스 트리를 VSCode 안에서 바로 확인할 수 있습니다. 서비스를 탐색하고, 동기화된 스크립트와 속성 파일을 열고, 동기화 상태를 추적하세요 — Studio로 전환할 필요 없이.
WEPPY Roblox Explorer는 WEPPY가 생성한 sync 데이터를 위한 companion VSCode 확장입니다. 기본 트리 탐색은 동기화된 파일만으로 동작하고, 로컬 MCP 서버가 실행 중이면 실시간 sync 상태와 direction 표시가 더 정확하게 반영됩니다.
