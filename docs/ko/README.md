...
### Basic과 Pro의 차이는?
Basic(무료)은 MCP 도구 실행과 단방향 동기화(Studio -> Local)를 포함합니다. Pro는 양방향 동기화, 대량 작업, 지형 생성, 공간 분석, 오디오/애니메이션 제어, 멀티 Place 지원을 추가합니다. [Pro 업그레이드 가이드](pro-upgrade.md)를 참고하세요.
